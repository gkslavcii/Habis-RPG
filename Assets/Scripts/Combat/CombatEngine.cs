...
namespace HabisRPG.Combat
{
    ...
    public class CombatEngine
    {
        ...
        public void InitCombat(
            List<CharacterController> playerParty,
            List<CharacterController> enemies)
        {
            AllUnits.Clear();
            RoundNumber = 1;
        ...
        public DamageEventArgs ExecuteSkill(SkillData skill, CombatUnit target)
        {
            ...
            // Apply status effect
            if (skill.AppliesEffect.HasValue && UnityEngine.Random.value <= skill.EffectChance)
            {
                ...
                var effect = StatusEffect.Create(
                    ...
                    GetEffectMagnitude(skill.AppliesEffect.Value)
                );
                ...
                OnStatusApplied?.Invoke(this, new CombatEventArgs
                {
                    Source = CurrentUnit.Character,
                    Target = effectTarget,
                    Message = $"{skill.AppliesEffect.Value} applied!"
                });
            }
